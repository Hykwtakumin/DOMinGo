...
import {getReservedDOM, setActionPairList, setReservedDOM} from "./libs/DOMinGoStorage";
...
export const setActionPair = async (targetElm: string, targetAction: DOMinGoAction) => {
...
window.onload = async () => {
    ...
    const localPairList = await chromep.storage.local.get("actionPairList");
    const localCustomThemeList = await chromep.storage.local.get("customThemeList");
    ...
    if (Object.keys(localReservedList).includes("reservedList") &&
        ...
        const customThemeList = Object.values(localCustomThemeList) as CustomTheme[];
        const pairList = Object.values(localPairList) as ActionPair[];
